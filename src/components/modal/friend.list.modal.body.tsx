...
const FriendListModalBody: React.FC<{ [index: string]: any }> = ({
  ...
  const [requestedFriendDataList, setRequestedFriendDataList] = useState<
    string[]
  >([]);
  ...
  useEffect(() => {
    ...
    const getFetchRequestedFriend = async () => {
      ...
      friendDataList.forEach((friendData) => {
        ...
        setRequestedFriendDataList([
          ...requestedFriendDataList,
          friendData.requestUser,
        ]);
      });
    };
      ...
      >
        <Flex col justifyCenter gridGap={"16px"}>
          <Table cellSpacing={"2px"}>
            <Thead>
              <Tr>
                <Th textAlign={"left"}>Pool Owner address</Th>
                <Th></Th>
              </Tr>
            </Thead>
            <Tbody>
              {walletConnected &&
                requestedFriendDataList.map((requestedAddr, index) => {
                  return (
                    <Tr key={index}>
